...
//
// Determinism: no Fortune parameter anywhere in this module. Arming and
// stage advance are both threshold checks on graph state (loyalty bp,
// tick arithmetic) -- fully deterministic by construction. (The plan's
// original "fortune jitters timing only" is dropped for this wave --
// controller resolution: reconsidered alongside Stage-2 schemes, if
// wanted, when Task 9 adds the 'scheme' kind.)
import type { Emitter, GraphDelta } from './events.js';
import { applyDeltas } from './events.js';
import type { WorldGraph } from './graph.js';
import { findEdge, getNode, nodesOfType, propStr } from './graph.js';
import { currentWant } from './spine.js';

/** kind is closed to 'restless' | 'scheme' so the type already documents
 *  Task 9's arrival: only 'restless' arcs are ever CREATED by this module
 *  today (the arm pass below never writes a 'scheme' key) -- 'scheme' is
 *  reserved so ReignState.arcs's shape doesn't change again once apparatus
 *  lands. Keyed in ReignState.arcs as `${kind}:${charId}`, so a character
 *  can carry a 'restless' arc and a 'scheme' arc at once: independent
 *  per-character slots, not a single arc-per-character constraint. */
...
export function advanceCharacterArcs(
  ...
  for (const key of Object.keys(arcs).sort()) {
    const arc = arcs[key]!;
    if (arc.kind !== 'restless') continue; // 'scheme' arcs: Task 9's territory, untouched here
    const { charId } = arc;
    ...
    if (stage === 2) {
      ...
      if (rivalId !== undefined) {
        // Informational only (no deltas) -- the offer targets the CURRENT
        // want. Task 9's counter-intel (obscure_records) makes a stale
        // offer instead by reading one step behind wantIndex.
        em.emit('arc.poach.bid', { data: { charId, byId: rivalId, offeredWant: want } });
      }
    } else {
      // stage 3: DEPARTURE, terminal. Spec §12: departed characters REMAIN
      // IN THE GRAPH -- only the loyalty edge is cut; nothing is removed.
      // Bundled into one event, same convention as ladder.ts's
      // demote-to-exile (N edge.removes + a node.set, all on one event).
      ...
      deltas.push({ op: 'node.set', id: charId, key: 'arc:restless', value: false });
      g = applyDeltas(g, deltas);
